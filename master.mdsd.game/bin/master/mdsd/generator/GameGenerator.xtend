...
class GameGenerator extends AbstractGenerator {
	...
	def createPathfindingFunc(Pathfinding pathfinding)'''
		...
		«elif(con.elseIfCondition)»
		
		«ENDFOR»
	'''
	
	def CharSequence elif(Condition elifCon)'''
		«ifHelper(elifCon, 'elif ')»:
			«conAction(elifCon.then)»
		
		«IF elifCon.elseIfCondition != null»
		«elif(elifCon.elseIfCondition)»
		«ENDIF»
	'''
	
	def String ifHelper(Condition con, String ifCon){
		var ifs = ifCon+ ifHelper2(con.ifCondition)
	...
	def String conAction(Action action){
		...
		if(action.charDec !=null){
			...
			else if(action.charDec.charAttResult.reference.characterId != null){
			...
			if(action.op != null){
				returnString = returnString + action.op.lop + ' '
			}
			returnString = returnString  + expression(action.ex)
	...
	def dispatch CharSequence expression(Expression i)'''
		«expression(i.tm)»'''
	
	def dispatch CharSequence expression(IntLiteral i)'''
		«i.value»'''
	
	def dispatch CharSequence expression(Operation i)'''
		«expression(i.left)» «i.op.lop» «expression(i.right)»'''
}
